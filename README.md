...
def plot_data(df, threshold, target_col, rows_to_display=None):
    ...
    for i, (var1_index, var2_index) in enumerate(display_range):
        ...
        if num_rows == 1:
            ...
            axes[i, 1].set_title(f'{x_axis_var} by {y_axis_var}', fontsize='11')
            axes[i, 1].set_xlabel(x_axis_var, fontsize=9)
            axes[i, 1].set_ylabel(y_axis_var, fontsize=9)
...
<!-- lost the code-->
<div style="text-align:center">
    <img src="README_files/tenure_salary_plot.png" alt="Alt Text">
</div>
In the histogram above, it is evident that employees with a 3-year tenure have the highest proportions in each salary category when compared to other tenure groups. Additionally, employees with shorter tenures are not disproportionately represented among lower-paid employees.
...
# First histogram
plt.subplot(1, 2, 1)
sns.histplot(data=df1, x='department', hue='promotion_last_5years', discrete=True, multiple='dodge', shrink=0.5)
plt.title('Counts of promoted or not by department', fontsize=14)
...
# Second histogram
plt.subplot(1, 2, 2)
sns.histplot(data=df1, x='department', hue='work_accident', discrete=True, multiple='dodge', shrink=0.5)
plt.title('Counts of work accident by department', fontsize=14)
plt.xticks(rotation=45)
...
- [Decision Tree](#Decision-Tree)
        ...
        - [Feature Engineered Decision Tree](#Feature-Engineered-Decision-Tree)
        - [Feature Engineered Random Forest](#Feature-Engineered-Random-Forest)
        - [Gradient Boosting Trees (GBT)](#Gradient-Boosting-Trees-(GBT))
-->
