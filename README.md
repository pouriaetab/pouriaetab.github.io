...
This dataset was provided as part of the Google Advanced Data Analytics capstone project, which was not included in the previous six courses. My goal is to exhibit my enthusiasm for data analysis by demonstrating an ability for uncovering insights and fostering innovation in the data space. This curiosity encompasses a variety of data types. My strength is in self-education, especially when encountering unfamiliar topics — I delve into academic papers, video tutorials, primary source documents, and related literature. To keep this report concise, not all my code is displayed. Including some code snippets here does lengthen the report, serving to showcase my coding style; however, the report could be significantly shorter without these codes.

This portfolio project is designed to partially reveal my thought process when analyzing a dataset. I am dedicated to ongoing learning and improvement. While this project is currently complete, I recognize its potential for further development. I am excited about the possibility of discussing or collaborating with you on future innovative projects.
    ...
    for i, (var1_index, var2_index) in enumerate(display_range):
        ...
        if num_rows == 1:
            ...
            if 'average_monthly_hours' in [x_axis_var, y_axis_var]:
                line_color = 'purple'
                annotation_text = 'Purple line: Avg. 169.33 hrs/month'
                ...
                if x_axis_var == 'average_monthly_hours':
                    ...
                    # Position the annotation inside the scatterplot
                    axes[i, 1].text(0.002, 0.995, annotation_text, color=line_color, transform=axes[i, 1].transAxes, 
...
```python
# Use previously made find_and_display_outliers
lower_limits, upper_limits = find_and_display_outliers(df1, display_info=False)
df1_logreg = df1_encode[(df1_encode['tenure'] >= lower_limits['tenure']) & (df1_encode['tenure'] <= upper_limits['tenure'])]
df1_logreg.head()
```
